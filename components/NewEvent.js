...
export default function NewEvent(){
    ...
    const addNewUpcomingEvent = async () => {
        const { error } = await supabase
        .from('upcomingEvent')
        .insert({eventName: evNameForm, clubName: clubNameForm, dateOn: dateForm, venue: venueForm});
    ...
    return(
        <div className="w-auto h-screen clear-both">
            <div className='text-whiteAlternate hover:text-blueAlternate1 mx-4 text-4xl font-montserrat mt-2 font-bold transition duration-300'>Add Details for an upcoming event</div>
            <form className="px-4">
                ...
                />
                <label for='clubNameElement' className="block text-whiteAlternate hover:text-blueAlternate1 text-xl mt-2 font-montserrat transition duration-300">Event Date (Day Month Year)</label>
                <input
                    type='text'
                    id='clubNameElement'
                    name='clubNameElement'
                ...
                />
                <button 
                    className="bg-whiteAlternate rounded-lg my-4 p-2 text-blueAccent2 hover:bg-blueAccent2 hover:text-whiteAlternate transition duration-300 block"
                    onClick={addNewUpcomingEvent}
                >
                    Add Event Details
                </button>
            </form>
            <div className='text-whiteAlternate hover:text-blueAlternate1 mx-4 text-xl font-montserrat mt-2 transition duration-300'>
                {updateStatus}
            </div>
        </div>
    );
